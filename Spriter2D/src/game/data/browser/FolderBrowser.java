...
public class FolderBrowser extends JFrame {
	...
	public String GetSavePath() {
		/*
		 * JFileChooser chooser = new JFileChooser(); File file = new
		 * File(chooser.getFileSystemView().getDefaultDirectory() .toString() +
		 * "\\9GatesData"); if (!file.exists()) file.mkdirs();
		 * 
		 * chooser.setCurrentDirectory(file);
		 * chooser.setAcceptAllFileFilterUsed(true); chooser.setFileFilter(new
		 * ImageFileFilter());
		 * 
		 * int retVal = chooser.showSaveDialog(this); if (retVal ==
		 * JFileChooser.APPROVE_OPTION) { File myFile =
		 * chooser.getSelectedFile(); return myFile.getAbsolutePath(); }
		 */return null;
