...
public class GifSequenceWriter {
	 ...
	 * writer.writeToSequence(firstImage); for (int i = 1; i < args.length - 1;
	 * i++) { BufferedImage nextImage = ImageIO.read(new File(args[i]));
	 * writer.writeToSequence(nextImage); }
	 * 
	 * writer.close(); output.close(); } else { System.out
	 ...
	 * @throws FileNotFoundException
	 */
	public static void CreateGif(String outputPath, BufferedImage[] images,
			int FrameTimeMS) throws FileNotFoundException, IOException {
		...
		GifSequenceWriter writer = new GifSequenceWriter(output,
				firstImage.getType(), FrameTimeMS, true);
